...
def procesar_frame():
    ...
    #Agrega esta línea AQUÍ para presiciones reales:
    frame_name = request.form.get('frame_name')  # ✅ para precisión real con etiquetas.json
    
    resultados = procesar_frame_yolo(frame_file, usuario_id = usuario_id, frame_name = frame_name)
...
def iniciar_monitoreo():
    ...
    session["monitoreo_finalizado"] = False  # ✅ reset
...
def finalizar_monitoreo():
    ...
    if session.get("monitoreo_finalizado", False):
        return jsonify({"error": "Monitoreo ya fue finalizado"}), 400
    ...
    monitoreo = Monitoreo(
        ...
        precision_fisura=datos["precision_fisura"],
    ...
    session["monitoreo_finalizado"] = True
